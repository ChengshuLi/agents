...
def main(_):
    class InferenceROSNode():
        def __init__(self):
            ...
            depth_sub = message_filters.Subscriber('/head_camera/depth_registered/hw_registered/image_rect', Image)
            rgb_sub = message_filters.Subscriber('/head_camera/rgb/image_rect_color', Image)

            ts = message_filters.ApproximateTimeSynchronizer([rgb_sub, depth_sub], queue_size=1, slop=0.5)
        ...
        def run(self):
            while not rospy.is_shutdown():
                ...
                #self.obs['depth'] = np.ones((1, 96, 128, 1)) * 3.0
