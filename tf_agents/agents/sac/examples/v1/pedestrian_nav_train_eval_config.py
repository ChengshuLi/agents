...
flags.DEFINE_string('train_config', 'yaml_configs/config.yaml',
...
flags.DEFINE_boolean('train_load_yaml', False,
                     'Train model with predefined yaml file')
FLAGS = flags.FLAGS
...
def main(_):
    ...
    # Load config file.
    print('-' * 10 + 'LOAD CONFIG FILE' + '-' * 10)
    print('{}'.format(FLAGS.train_config))
    with open(FLAGS.train_config, 'r') as configfile:
        config_dict = yaml.load(configfile)
    for k, v in config_dict.items():
        print('{}: {}'.format(k, v))
    print('-' * 30)
    
    # Run train_eval with loaded parameters.
    # For now, all the fields are REQUIRED.
    # Environment type, env_mode, num_parallel_environmetns, train/eval are directly parsed from FLAGS.
    #TODO: Maybe allow overwriting with FLAGS?
    train_eval(
        root_dir=config_dict.get('root_dir'),
        gpu=config_dict.get('gpu_g'),
        env_load_fn=lambda model_id, mode, device_idx: suite_gibson.load(
            config_file=config_dict.get('config_file'),
            model_id=model_id,
            collision_reward_weight=config_dict.get('collision_reward_weight'),
            env_type=config_dict.get('env_type'),
            env_mode=mode,
            action_timestep=config_dict.get('action_timestep'),
            physics_timestep=config_dict.get('physics_timestep'),
            device_idx=device_idx,
            random_position=FLAGS.random_position,
            fixed_obstacles=FLAGS.fixed_obstacles,
            pedestrians=FLAGS.pedestrians,
            random_obstacles=FLAGS.random_obstacles,
            random_height=config_dict.get('random_height'),
        ),
        model_ids=FLAGS.model_ids,
        eval_env_mode=FLAGS.env_mode,
        num_iterations=config_dict.get('num_iterations'),
        conv_layer_params=config_dict.get('conv_layer_params'),
        encoder_fc_layers=config_dict.get('encoder_fc_layers'),
        actor_fc_layers=config_dict.get('actor_fc_layers'),
        critic_obs_fc_layers=config_dict.get('critic_obs_fc_layers'),
        critic_action_fc_layers=config_dict.get('critic_action_fc_layers'),
        critic_joint_fc_layers=config_dict.get('critic_joint_fc_layers'),
        initial_collect_steps=config_dict.get('initial_collect_steps'),
        collect_steps_per_iteration=config_dict.get('collect_steps_per_iteration'),
        num_parallel_environments=FLAGS.num_parallel_environments,
        replay_buffer_capacity=config_dict.get('replay_buffer_capacity'),
        train_steps_per_iteration=config_dict.get('train_steps_per_iteration'),
        batch_size=config_dict.get('batch_size'),
        actor_learning_rate=config_dict.get('actor_learning_rate'),
        critic_learning_rate=config_dict.get('critic_learning_rate'),
        alpha_learning_rate=config_dict.get('alpha_learning_rate'),
        gamma=config_dict.get('gamma'),
        num_eval_episodes=config_dict.get('num_eval_episodes'),
        eval_interval=config_dict.get('eval_interval'),
...
if __name__ == '__main__':
    # No longer necessary since we can load them from YAML file
