...
def train_eval(
    ...
    with tf.compat.v2.summary.record_if(
        ...
        print('PREPROCESSING ARCHITECTURE: {}'.format(preprocessing_layers_architecture))
        print('preprocessing layers: {}'.format(preprocessing_layers))
        if len(sensor_inputs) > 1:
            preprocessing_combiner = tf.keras.layers.Concatenate(axis=-1)
        else:
            preprocessing_combiner = None

        print('CREATE ACTOR NETWORK')
        ...
        print('ACTOR NET: {}'.format(actor_net))

        print('CREATE CRITIC NETWORK')
        ...
        print('CRITIC NET: {}'.format(critic_net))

        print('CREATE AGENT')
        print('TIME STEP SPEC: {}'.format(time_step_spec))
        print('alpha learning rate: {}'.format(alpha_learning_rate))
        ...
        print('TF AGENT: {}'.format(tf_agent))
...
def main(_):
    ...
    with open(env_config_file, 'r') as env_configfile:
        env_config = yaml.load(env_configfile)
